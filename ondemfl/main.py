...
if __name__ == "__main__":
    ...
    baseline_dir_name = f'nc_{args.num_clients}-a_{args.alpha}-tep_{args.tm_local_ep}-tbs_{args.tm_local_bs}-dep_{args.dm_local_ep}-dbs_{args.dm_local_bs}-dmc_{args.dm_criterion}'
    dir_name = f'nc_{args.num_clients}-op_{args.target_dist_op}-ss_{args.subset_size}-a_{args.alpha}-tep_{args.tm_local_ep}-tbs_{args.tm_local_bs}-dep_{args.dm_local_ep}-dbs_{args.dm_local_bs}-init_{args.init_round}-predinit_{args.client_pred_round}'
    ...
    system_config = {
        'is_mp': args.mp, 'device': device, 'seed': args.seed, 'log_dir': log_path, 'baseline_dir_name': baseline_dir_name, 'time_config': time_config
    }
    ...
    arguments = {
        ...
        'client_pred_round': args.client_pred_round,
        ...
        'target_dist_op': args.target_dist_op,
        'dm_model_idx': args.dm_model_idx,
        'dm_pred_method': args.dm_pred_method,
    }
    if args.wandb == True:
        wandb.init(project = 'on-demand-fl', config = arguments, name = f"{dir_name}")
