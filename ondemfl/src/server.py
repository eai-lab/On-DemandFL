...
class Server(object):
    ...
    def __init__(self, train_dataset, test_dataset_list, arguments):
        ...
        self.client_pred_round = arguments['client_pred_round']
    ...
    def setup(self, **init_kwargs):
        ...
        self.client_dist = np.array([pred.numpy() for pred in result[self.dm_pred_method][self.client_pred_round][self.dm_model_idx]])
    ...
    def fit(self):
        ...
        for r in range(self.check_point, self.num_rounds):
            ...
            if self.wandb:
                wandb.log({f'OnDemFL-{self.target_dist_op}': test_accuracy, 'round': self.init_round + self.check_point + self._round})
