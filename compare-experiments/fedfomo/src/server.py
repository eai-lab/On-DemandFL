...
class Server(object):
    ...
    def __init__(self, writer, train_dataset, val_dataset, test_dataset_list, fed_config={}, data_config={}, model_config={}, system_config={}):
        self.clients = None
        self._round = 0
        self.writer = writer
        ...
        self.uploaded_ids = []
        self.M = min(5, self.fraction*self.num_clients)
        # self.M = self.fraction*self.num_clients
    ...
    def setup(self, **init_kwargs):
        ...
        self.clients = self.create_clients(self.train_dataset, self.val_dataset)
    ...
    def create_clients(self, local_datasets, local_val_dataset):
        """Initialize each Client instance."""
        clients = []
        for k in range(0, self.num_clients):
            client = Client(client_id=k, local_data=local_datasets[k], val_data=local_val_dataset[k], 
                            device=self.device, num_clients=self.num_clients, log_path=f"{self.log_dir}")
    ...
    def transmit_model(self, sampled_client_indices=None):
        ...
        if sampled_client_indices is None:
            ...
            for idx in tqdm(sampled_client_indices, leave=False):
                ...
                indices = torch.topk(self.P[idx], M_).indices.tolist()
                
                print('transmit model (indices): ', indices)
                
                send_ids = []
                send_models = []
                for i in indices:
                    send_ids.append(i)
                    send_models.append(self.clients[i].model)
                    # self.clients[i].send_ids.append(i)
                    # self.clients[i].send_models.append(self.clients[i].task_model)
                self.clients[idx].received_ids = send_ids
                self.clients[idx].received_models = send_models
    ...
    def train_federated_model(self):
        ...
        self.received_models(sampled_client_indices)

    def received_models(self, sampled_client_indices):
        assert (len(sampled_client_indices) > 0)
        
        active_clients_indices = random.sample(
            sampled_client_indices, int(self.fraction*self.num_clients))

        self.uploaded_ids = []
        # top_samples = 0
        for client_idx in active_clients_indices:
            self.uploaded_ids.append(self.clients[client_idx].id)
            # tot_samples += client.train_samples
            # self.client_models[client.id] = copy.deepcopy(client.model)
            self.P[client_idx] += self.clients[client_idx].weight_vector
            # self.P[client.id] += self.clients[client_idx].weight_vector
    ...
    def fit(self):
        ...
        with pool.ThreadPool(10) as workhorse:
            ...
            # acc_list = workhorse.map(client_test, [r in range(self.num_clients)])
        for i in range(self.num_clients):
