...
def partition_with_dirichlet_distribution(dataset_name, data, targets, class_to_idx, num_client, alpha, transform, seed):
    ...
    # add shuffle for FedFomo validation dataset 
    for i in range(0, num_client):
        random.shuffle(client_data_indecies[i])

    def softmax(arr):
        return torch.Tensor(arr/np.sum(arr))

    splited_client_train_dataset = []
    splited_client_val_dataset = []
    for idx in range(0, num_client):
        client_data_idx = client_data_indecies[idx]
        
        split_pivot = int(len(client_data_idx)*0.8)
        
        if split_pivot == 0:
            splited_client_train_dataset.append(
                ClientDataset(
                    data = data[client_data_idx[:]], # client_datas[idx] if type(data[0]) == str else torch.Tensor(client_datas[idx]),
                    targets = torch.Tensor(targets)[client_data_idx[:]],
                    dist = softmax(client_dist[idx]),
                    class_to_idx = class_to_idx,
                    transform=transform
                )
            )
            splited_client_val_dataset.append(
                ClientDataset(
                    data = data[client_data_idx[:]], # client_datas[idx] if type(data[0]) == str else torch.Tensor(client_datas[idx]),
                    targets = torch.Tensor(targets)[client_data_idx[:]],
                    dist = softmax(client_dist[idx]),
            ...
            splited_client_train_dataset.append(
                ClientDataset(
                    data = data[client_data_idx[:int(len(client_data_idx)*0.8)]], # client_datas[idx] if type(data[0]) == str else torch.Tensor(client_datas[idx]),
                    targets = torch.Tensor(targets)[client_data_idx[:int(len(client_data_idx)*0.8)]],
                    dist = softmax(client_dist[idx]),
                    class_to_idx = class_to_idx,
                    transform=transform
                )
    ...
    return splited_client_train_dataset, splited_client_val_dataset
...
def prepare_dataset(seed, dataset_name, target_dist_list, num_client, alpha):
    ...
    if hasattr(torchvision.datasets, dataset_name):
        ...
        partitioned_train_dataset, partitioned_val_dataset = partition_with_dirichlet_distribution(dataset_name, train_dataset.data, train_dataset.targets, train_dataset.class_to_idx, num_client, alpha, transform, seed)
        
    return partitioned_train_dataset, partitioned_val_dataset, test_dataset_list